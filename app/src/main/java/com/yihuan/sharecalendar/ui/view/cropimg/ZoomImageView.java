package com.yihuan.sharecalendar.ui.view.cropimg;
...
public class ZoomImageView extends ImageView implements
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isCanDrag) {
                    if (getDrawable() != null) {
                        ...
                        if (rectF.width() <= getWidth() - mHOffset * 2) {
                            dx = 0;
                        }
    ...
    public void onGlobalLayout() {
        if (isFirst) {
            ...
            if (d == null)
                return;
//            mVOffset = (getHeight() - (getWidth() - 2 * mHOffset)) / 2;
            ...
            float scale = 1.0f;
            if (dw < getWidth() - mHOffset * 2
                    && dh > getHeight() - mVOffset * 2) {
                scale = (getWidth() * 1.0f - mHOffset * 2) / dw;
            }

            if (dh < getHeight() - mVOffset * 2
                    && dw > getWidth() - mHOffset * 2) {
                scale = (getHeight() * 1.0f - mVOffset * 2) / dh;
            }

            if (dw < getWidth() - mHOffset * 2
                    && dh < getHeight() - mVOffset * 2) {
                float scaleW = (getWidth() * 1.0f - mHOffset * 2)
                        / dw;
                float scaleH = (getHeight() * 1.0f - mVOffset * 2) / dh;
                scale = Math.max(scaleW, scaleH);
            }

            float sw = (float) (width - 2 * mHOffset) / (float) dw;
    ...
    public Bitmap cropBitmap() {
        ...
        draw(canvas);
        return Bitmap.createBitmap(bitmap, mHOffset,
                mVOffset, getWidth() - 2 * mHOffset,
                getHeight() - 2 * mVOffset);
    }
    ...
    private void checkBorder() {
        ...
        if (rect.width() + 0.01 >= width - 2 * mHOffset) {
            if (rect.left > mHOffset) {
                deltaX = -rect.left + mHOffset;
            }
            if (rect.right < width - mHOffset) {
                deltaX = width - mHOffset - rect.right;
            }
        }
